...
saTemper <- function(costFunc,init,tempVect=NA,...,control=list()) {
  ...
  if(!haveChains) {
    for(m in 1:numTemp) {
      chainList[[m]] <- samMCMC::saMetrop(costFunc,as.vector(X_0[m,]),tempVect[m],...,control=metropControl)
    }
  ...
  while(!done) {
    ...
    for(m in 1:numTemp) {
      chainList[[m]] <- samMCMC::saMetrop(costFunc,chainList[[m]],tempVect[m],...,control=metropControl)
    }
    cycle <- cycle + 1
